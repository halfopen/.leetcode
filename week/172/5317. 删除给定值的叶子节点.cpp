### [5317\. 删除给定值的叶子节点](https://leetcode-cn.com/contest/weekly-contest-172/problems/delete-leaves-with-a-given-value/)
...
public:
    TreeNode* removeLeafNodes(TreeNode* root, int target) {
        while(dfs(root, target)) {
            //cout<<"---del---" <<endl;
            if (nullptr!=root && nullptr==root->left && nullptr==root->right && target==root->val) return nullptr;
            del(root, target, nullptr);
        }
        return root;
    }
    ...
    void del(TreeNode* root, int target, TreeNode* parent) {
        ...
        del(root->left, target, root);
        del(root->right, target, root);
    }
};